...
export default function checkConfigFile(id) {
    ...
    if (config.capabilities && config.capabilities.deleteIdProp) {
        if (!propExists(id, config.capabilities.deleteIdProp)) {
    ...
    if (config.filtering) {
        if (config.filtering.filterFields) {
            for (let i = 0, e = config.filtering.filterFields.length; i < e; i++) {
                if (!propExists(id, config.filtering.filterFields[i])) {
        ...
        if (config.filtering.searchFields) {
            for (let i = 0, e = config.filtering.searchFields.length; i < e; i++) {
                if (!propExists(id, config.filtering.searchFields[i])) {
    ...
    if (config.defaultSort && config.defaultSort.field) {
        if (!propExists(id, config.defaultSort.field)) {
    ...
    if (config.archive && config.archive.field) {
        if (!propExists(id, config.archive.field)) {
            ...
            const archiveField = getPropByName(id, config.archive.field);
    ...
    if (config.modalGrid) {
        for (let i = 0, e = config.modalGrid.length; i < e; i++) {
            for (let x = 0, y = config.modalGrid[i].length; x < y; x++) {
                if (!propExists(id, config.modalGrid[i][x])) {
...
function propExists(id, p) {
    if (p === 'sort' || p === 'id') {
        return true;
    }

    if (!getPropByName(id, p)) {
        return false;
    }
